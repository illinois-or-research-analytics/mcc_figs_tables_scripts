...
plot_panel <- function(data, title, subtitle, filename, facet_scales = "free_y") {
  p <- ggplot(data, aes(x = sim_year, y = mean_rate,
    ...
    facet_wrap(~ cartel_size, scales = facet_scales, ncol = 2) +
...
# --- Plot 1: cartel/control authors (target_cartel != -1), fixed y-axis ---
plot_panel(
