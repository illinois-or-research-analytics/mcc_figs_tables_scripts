...
df <- bind_rows(df_318, df_322) %>%
  mutate(
    model = recode(model, "control" = "ctrl", "null" = "cartel-r", "phenotype" = "cartel-p"),
    model = factor(model, levels = c("ctrl", "cartel-r", "cartel-p")),
    size = factor(size, levels = c(5, 25, 125, 250, 500, 1000)),
...
p <- ggplot(df, aes(x = size, y = community_size, fill = experiment)) +
  ...
  geom_text(aes(label = paste0("k", k)),
            ...
            vjust = -0.3, size = 3.5, fontface = "plain") +
...
​
ggsave(file.path(basedir, "corrected_csearch_on_sasca.png"), p, width = 14, height = 6, dpi = 150)
ggsave(file.path(basedir, "corrected_csearch_on_sasca.eps"), p, width = 14, height = 6, dpi = 150)
ggsave(file.path(basedir, "corrected_csearch_on_sasca.pdf"), p, width = 14, height = 6, dpi = 150)
cat("Saved: corrected_csearch_on_sasca.png, corrected_csearch_on_sasca.eps, corrected_csearch_on_sasca.pdf\n")
