  ...
  for (model in models) {
    for (size in sizes) {
      for (rep in reps) {
        ...
        # Filter to cartel agents only — small lookup table
        ...
        target_lookup   <- cartel_nodes[, .(node_id, tyear = year,
                                            tauthor_id = author_id)]
        cartel_node_ids <- target_lookup[, node_id]
        setkey(target_lookup, node_id)

        # Pre-filter edgelist to cartel targets, then keyed binary search join
        el           <- fread(el_path)
        el_filtered  <- el[target %in% cartel_node_ids]
        setkey(el_filtered, target)
        cartel_cites <- el_filtered[target_lookup, on = c(target = "node_id"),
                                    nomatch = 0]

        df <- cartel_cites[, .N, by = .(tauthor_id, tyear)]
        df[, condition        := cond]
...
# Citations per cartel author per year, per replicate
...
out <- "fig6_citations_per_author_v2.pdf"
