...
# --- Compute fold enrichment for top 1% only ---
...
p <- ggplot(dt, aes(x = condition, y = fold_top1,
                    color = condition, group = condition)) +
  geom_hline(yintercept = 1, linetype = "dashed",
             color = "grey40", linewidth = 0.5) +
  geom_segment(aes(xend = condition, y = 0, yend = fold_top1),
               position = dodge, linewidth = 0.9) +
...
ggsave("cartel_top1pct_lollipop.png",
       plot = p, width = 10, height = 7, dpi = 300)
message("Saved: cartel_top1pct_lollipop.png")

# --- Vector formats (uncomment to use) ---
# ggsave("cartel_top1pct_lollipop.pdf",
#        plot = p, width = 10, height = 7, device = "pdf")
# ggsave("cartel_top1pct_lollipop.eps",
